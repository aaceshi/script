...
<script language="JavaScript" type="text/javascript" src="https://cdn.bootcss.com/jquery/1.7/jquery.min.js"></script>
...
<div id="page">
	<table>
		<tr>
			...
			<th class="w_top"><a href="#w_MySQLI">MySQLI检测</a></th>
...
<table>
  ...
  <tr>
    ...
    <td width="18%">
		...
		$disFuns=get_cfg_var("disable_functions");
		?>
    <?php echo preg_match("/phpinfo/", $disFuns) ? '<font color="red">×</font>' : "<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>"; ?>
    </td>
...
<table>
  ...
  <tr>
    <td>MCrypt加密处理：</td>
    <td><?php echo isfun("mcrypt_generic");?></td>
    <td>哈稀计算：</td>
...
<table>
  ...
  <tr align="center">
    <td align="left">埃及 CitynetHost</a></td>
    <td>0.343秒</td>
    <td>0.761秒</td>
    <td>0.023秒</td>
  ...
  <tr align="center">
    <td align="left">美国 IXwebhosting</td>
    <td>0.535秒</td>
    <td>1.607秒</td>
    <td>0.058秒</td>
...
<table>
  ...
  if ($_POST['act'] == 'MySQLI检测') {
  	if(function_exists("mysqli_close")==1) {
  		$link = @mysqli_connect("$host","$login","$password","$dbname","$port","$dbsocket");
  		if ($link){
  			$v = @mysqli_get_server_info($link);
  			echo "<script>alert('连接到MySQLI数据库正常，版本 v{$v}')</script>";
  		} else {
  			echo "<script>alert('无法连接到MySQLI数据库！')</script>";
  		}
  	} else {
  		echo "<script>alert('服务器不支持MySQLI数据库！')</script>";
  	}
  }
	?>
<a name="w_function"></a>
...
<table>
  ...
  ?>
	...
	<table>
		<tr>
			<td class="w_foot">&copy;<?php echo $year.' '.$title;?></td>
